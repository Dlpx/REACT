...
const Item = ({prod}) => {
    return(
        <Link className="producto" to={`/mostrar/${Number(prod.id)}`}>
            ...
            <p className="prodPrecio">Precio: { new Intl.NumberFormat('en-AR', {style: 'currency', currency: 'ARS'}).format(prod.precio) }</p>
        </Link>
    )
}
